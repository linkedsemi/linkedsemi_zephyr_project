...
/* ---- perf_write: fstestisoc perf write <path> <size_kb> [repeat] [buf_size] ---- */
static int cmd_perf_write(const struct shell *sh, size_t argc, char **argv)
{
	...
	if (argc < 3) {
		shell_error(sh, "Usage: fstestisoc perf write <path> <size_kb> [repeat] [buf_size]");
		return -EINVAL;
	}
	...
	shell_print(sh, "=== ISO C Write Performance Test ===");
...
/* ---- perf_read: fstestisoc perf read <path> [repeat] [buf_size] ---- */
static int cmd_perf_read(const struct shell *sh, size_t argc, char **argv)
{
	...
	if (argc < 2) {
		shell_error(sh, "Usage: fstestisoc perf read <path> [repeat] [buf_size]");
		return -EINVAL;
	}
	...
	fp = fopen(argv[1], "rb");
	if (fp == NULL) {
		shell_error(sh, "fopen(%s) failed: errno=%d", argv[1], errno);
		return -errno;
	}

	if (fseek(fp, 0, SEEK_END) != 0) {
		shell_error(sh, "fseek(%s) failed: errno=%d", argv[1], errno);
		fclose(fp);
		return -errno;
	}
	...
	file_size = ftell(fp);
	if (file_size < 0) {
		shell_error(sh, "ftell(%s) failed: errno=%d", argv[1], errno);
		fclose(fp);
		return -errno;
	}
	...
	shell_print(sh, "=== ISO C Read Performance Test ===");
	shell_print(sh, "  File:      %s", argv[1]);
	...
	for (loop = 0; loop < repeat; loop++) {
		...
		if (total_read != (uint32_t)file_size) {
			shell_error(sh, "Short read: expected %ld, got %u",
...
SHELL_STATIC_SUBCMD_SET_CREATE(sub_fs_perf,
	SHELL_CMD_ARG(write, NULL,
		"ISO C write performance test: fstestisoc perf write <path> <size_kb> [repeat] [buf_size]",
		cmd_perf_write, 3, 2),
	SHELL_CMD_ARG(read, NULL,
		"ISO C read performance test: fstestisoc perf read <path> [repeat] [buf_size]",
		cmd_perf_read, 2, 2),
...
SHELL_STATIC_SUBCMD_SET_CREATE(sub_fs,
	SHELL_CMD(perf, &sub_fs_perf,
		"ISO C file read/write performance tests", NULL),
	SHELL_SUBCMD_SET_END
);

SHELL_CMD_REGISTER(fstestisoc, &sub_fs, "ISO C file system commands", NULL);
