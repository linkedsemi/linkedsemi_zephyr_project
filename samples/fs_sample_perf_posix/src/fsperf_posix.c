...
/* ---- perf_write: fstestposix perf write <path> <size_kb> [repeat] [buf_size] ---- */
static int cmd_posix_write(const struct shell *sh, size_t argc, char **argv)
{
	int fd;
	uint32_t size_kb;
	uint32_t repeat = 1;
	...
	if (argc < 3) {
		shell_error(sh, "Usage: fstestposix perf write <path> <size_kb> [repeat] [buf_size]");
		return -EINVAL;
	}
	...
	shell_print(sh, "=== POSIX Write Performance Test ===");
	...
	for (loop = 0; loop < repeat; loop++) {
		...
		if (fd < 0) {
			shell_error(sh, "Failed to open %s (errno=%d)", argv[1], errno);
			free(buf);
			return -ENOEXEC;
		}
		...
		while (remaining > 0) {
			...
			if (written < 0) {
				shell_error(sh, "Write failed at offset %u (errno=%d)",
					    total_bytes - remaining, errno);
				close(fd);
				free(buf);
				return -ENOEXEC;
			}
...
/* ---- perf_read: fstestposix perf read <path> [repeat] [buf_size] ---- */
static int cmd_posix_read(const struct shell *sh, size_t argc, char **argv)
{
	struct stat st;
	int fd;
	uint32_t repeat = 1;
	...
	if (argc < 2) {
		shell_error(sh, "Usage: fstestposix perf read <path> [repeat] [buf_size]");
		return -EINVAL;
	}
	...
	if (stat(argv[1], &st) != 0) {
		shell_error(sh, "stat(%s) failed: errno=%d", argv[1], errno);
		return -errno;
	}

	if (!S_ISREG(st.st_mode)) {
		shell_error(sh, "%s is not a regular file", argv[1]);
		return -ENOENT;
	}
	...
	shell_print(sh, "=== POSIX Read Performance Test ===");
	shell_print(sh, "  File:      %s", argv[1]);
	...
	for (loop = 0; loop < repeat; loop++) {
		...
		while (1) {
			...
			if (nread < 0) {
				shell_error(sh, "Read failed (errno=%d)", errno);
				close(fd);
				free(buf);
				return -ENOEXEC;
			}
...
SHELL_STATIC_SUBCMD_SET_CREATE(sub_fs_perf,
	SHELL_CMD_ARG(write, NULL,
		"POSIX write performance test: fstestposix perf write <path> <size_kb> [repeat] [buf_size]",
		cmd_posix_write, 3, 2),
	SHELL_CMD_ARG(read, NULL,
		"POSIX read performance test: fstestposix perf read <path> [repeat] [buf_size]",
		cmd_posix_read, 2, 2),
...
SHELL_STATIC_SUBCMD_SET_CREATE(sub_fs,
	SHELL_CMD(perf, &sub_fs_perf,
		"POSIX file read/write performance tests", NULL),
	SHELL_SUBCMD_SET_END
);

SHELL_CMD_REGISTER(fstestposix, &sub_fs, "POSIX file system commands", NULL);
