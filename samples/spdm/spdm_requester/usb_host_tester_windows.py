...
#
# PC 是 SPDM 响应方，测设备固件例程 spdm_requester（设备是请求方）。
# 先运行本文件，再插板/复位。设备 USB 配置后约 5 秒开始 GET_VERSION。
# 本文件已包含 USB 后端 + SPDM 协议，无需其它 .py。
#
#   pip install pyusb cryptography libusb-package
#   Bind WinUSB with Zadig (VID 2FE3 PID 0001), then unplug/replug.
#
...
if sys.platform != "win32":
    sys.stderr.write("This is the Windows tester. On Linux run usb_host_tester_linux.py\n")
    sys.exit(1)

HERE = os.path.dirname(os.path.abspath(__file__))

def _libusb_dll_dirs():
    return [
        HERE,
        os.getcwd(),
        os.path.join(HERE, "libusb"),
        os.path.join(HERE, "windows"),
    ]


def _prepare_libusb_dll():
    found = None
    for d in _libusb_dll_dirs():
        dll = os.path.join(d, "libusb-1.0.dll")
    ...
    if found is None:
        try:
            ...
            packaged = libusb_package.get_library_path()
            if packaged and os.path.isfile(packaged):
                found = os.path.dirname(os.path.abspath(packaged))
        except Exception:
            found = None
    if found is None:
        raise SystemExit(
            "libusb-1.0.dll not found.\n"
            "Option A (recommended):  python -m pip install libusb-package\n"
            "Option B: download https://github.com/libusb/libusb/releases\n"
            "          and put the 64-bit libusb-1.0.dll next to this script."
        )
    os.environ["PATH"] = found + os.pathsep + os.environ.get("PATH", "")
    if hasattr(os, "add_dll_directory"):
        os.add_dll_directory(found)
    return os.path.join(found, "libusb-1.0.dll")


_LIBUSB_DLL = _prepare_libusb_dll()
...
def _windows_backend():
    backend = usb.backend.libusb1.get_backend(find_library=lambda _: _LIBUSB_DLL)
    if backend is None:
        raise SystemExit(
            f"pyusb could not load {_LIBUSB_DLL}. Use the 64-bit DLL that matches "
            "this Python (64-bit python.exe -> 64-bit libusb-1.0.dll)."
        )
    return backend
    ...
    def connect(self):
        ...
        if self.device is None:
            raise ValueError(
                ...
                "Windows checklist:\n"
                "  1. Board enumerated? Device Manager should list it.\n"
                "  2. Bind WinUSB with Zadig (List All Devices, VID 2FE3 PID 0001).\n"
                "  3. Unplug and replug USB after installing WinUSB.\n"
                "  4. libusb-1.0.dll is next to this script (already found: "
                f"{_LIBUSB_DLL})."
            )
        ...
        try:
            ...
            raise RuntimeError(
                "Cannot claim USB interface. Confirm the device is bound to WinUSB "
                "(not usbser / WinUSB missing), close other programs using it, "
                "then unplug/replug."
            ) from err
        self.arm_bulk_pipes()
        print(
            f"Connected (Windows/WinUSB) to {self.vid:04x}:{self.pid:04x} "
...
def main() -> None:
    print(f"Using {_LIBUSB_DLL}")
    ensure_host_responder_certs()
    usb_dev = HostUSBDevice(VID, PID)
    try:
        usb_dev.connect()
        run_spdm_responder(usb_dev)
    finally:
        usb_dev.disconnect()
