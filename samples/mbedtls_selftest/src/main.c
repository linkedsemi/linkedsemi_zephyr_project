...
#undef CHECK_PADDING_UNSIGNED
     ...
     * Note: MBEDTLS_PLATFORM_SNPRINTF_ALT is not set in config-tls-generic.h,
     * so mbedtls_snprintf defaults to a dummy. We warn but don't abort.
     */
    if (run_test_snprintf() != 0) {
        mbedtls_printf("  [warn] snprintf implementation is not configured (MBEDTLS_PLATFORM_SNPRINTF_ALT not set)\n");
    }
