...
static const char echoserverBanner[] = "wolfSSH Server\n";
...
__maybe_unused static int process_bytes(thread_ctx_t* threadCtx,
...
static int ssh_worker(thread_ctx_t* threadCtx)
{
    ...
    wolfSSH_DoModes(ssh->modes, ssh->modesSz, childFd);
    {
        ...
        while (1) {
            ...
            WS_SOCKET_T maxFd;
            int cnt_r;
            __maybe_unused int cnt_w;

            FD_ZERO(&readFds);
            FD_SET(sshFd, &readFds);
            FD_SET(sh_ssh->tx_efd, &readFds);
            maxFd = sshFd > sh_ssh->tx_efd ? sshFd : sh_ssh->tx_efd;
#ifdef WOLFSSH_SHELL
            if (!threadCtx->echo) {
                ...
                if (cnt_r < 0) {
                    ...
                    if (rc == WS_CHAN_RXD) {
                        if (lastChannel == shellChannelId) {
                            size_t channel_buf_free = ring_buf_space_get(sh_ssh->rx_buf);
                            int channel_buf_size = 0;
                            uint8_t *channel_buf = NULL;
                            if (channel_buf_free > 0) {
                                do {
                                    channel_buf_size = ring_buf_put_claim(sh_ssh->rx_buf, &channel_buf, channel_buf_free);
                                    if (channel_buf_size) {
                                        ...
                                        if (cnt_w <= 0)
                                            break;
                                        #endif
                                        int ret = ring_buf_put_finish(sh_ssh->rx_buf, cnt_r);
                                        __ASSERT_NO_MSG(0 == ret);
                                        channel_buf_free = ring_buf_space_get(sh_ssh->rx_buf);
                                        //DBG_DEC(cnt_r)
                                    }
                                } while ((cnt_r > 0) && (channel_buf_free > 0));
            ...
            if (FD_ISSET(sh_ssh->tx_efd, &readFds)) {
                uint64_t msg;
                int ret = eventfd_read(sh_ssh->tx_efd, &msg);
                if (ret) {
                    __ASSERT_NO_MSG(0);
                }
                size_t channel_buf_used = ring_buf_size_get(sh_ssh->tx_buf);
                size_t channel_buf_size = 0;
                uint8_t *channel_buf = NULL;
                if (channel_buf_used > 0) {
                    do {
...
THREAD_RETURN WOLFSSH_THREAD echoserver_test(void* args)
{
    ...
    int echo = 0;
    int ch;
    word16 port = CONFIG_WOLFSSHPORT;
    char* readyFile = NULL;
...
#if 1
        char* z_argv[] = {""};
        return wolfSSH_Echoserver(1, z_argv);
#else
...
static void ssh_server_backend(void *dummy1, void *dummy2, void *dummy3)
{
    char* z_argv[] = {""};
    wolfSSH_Echoserver(1, z_argv);
    /* unreachable */
}


static int ssh_shell_init(const struct shell_transport *transport, const void *config,
        shell_transport_handler_t evt_handler, void *context)
{
    ...
    k_mutex_init(&sh_ssh->rx_lock);
    k_mutex_init(&sh_ssh->tx_lock);
...
static int ssh_shell_uninit(const struct shell_transport *transport)
{
    return 0;
}
...
static int ssh_shell_write(const struct shell_transport *transport, const void *data, size_t length, size_t *cnt)
{
    int ret = 0;
    ...
    int channel_buf_free = 0;
    for(;;) {
        channel_buf_free = ring_buf_space_get(sh_ssh->tx_buf);
        if (channel_buf_free < length) {
            ret = eventfd_write(sh_ssh->tx_efd, length);
            if (ret) {
                __ASSERT_NO_MSG(0);
            }
            k_yield();
    ...
    if (length != ring_buf_put(sh_ssh->tx_buf, data, length)) {
        printf("tx bytes dropped from ring buf\n");
        __ASSERT_NO_MSG(0);
    }
...
static int ssh_shell_read(const struct shell_transport *transport, void *data, size_t length, size_t *cnt)
{
    if (sh_ssh == NULL) {
        return -ENODEV;
    }
    ...
    int channel_buf_used = ring_buf_size_get(sh_ssh->rx_buf);
    if (0 == channel_buf_used) {
        k_mutex_unlock(&sh_ssh->rx_lock);
        goto no_data;
    }
    *cnt = ring_buf_get(sh_ssh->rx_buf, data, length);
...
const struct shell_transport_api shell_ssh_transport_api = {
    .init = ssh_shell_init,
    .uninit = ssh_shell_uninit,
    .enable = ssh_shell_enable,
